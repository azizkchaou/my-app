...
export const sendCheckRiskAlerts = inngest.createFunction(
  {
    id: "send-check-risk-alerts",
    name: "Send Check Risk Alerts",
  },
  { cron: "0 8 * * *" },
  async ({ step }) => {
    const checks = await step.run("fetch-pending-issued-checks", async () => {
    ...
    for (const check of checks) {
      await step.run(`check-risk-alert-${check.id}`, async () => {
        const defaultAccount = check.user.accounts[0];
